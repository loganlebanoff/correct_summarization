...
def get_first_nonpronoun_mention_less_than_5_words(mentions):
    for m in mentions:
        tokens = m['text'].split(' ')
        contains_punctuation = any([util.is_punctuation(token) for token in tokens])
        if m['type'] != 'PRONOMINAL' and (int(m['endIndex']) - int(m['startIndex']) <= 5) and not contains_punctuation:
            return m
    return None
...
def replace_corefs(article_sent_tokens, corefs, replace_rep_mention=False):
    coref_article_sent_tokens = copy.deepcopy(article_sent_tokens)
    corefs_skipped = 0
    total_corefs = len(corefs)
    for mentions in corefs:
        rep_mention = get_first_nonpronoun_mention_less_than_5_words(mentions)
        # if rep_mention is None:
        #     tqdm.write('<None>' + ' ------- ' + get_rep_mention(mentions)['text'])
        # elif not rep_mention['isRepresentativeMention']:
        #     tqdm.write(rep_mention['text'] + ' ------- ' + get_rep_mention(mentions)['text'])
        # else:
        #     tqdm.write(rep_mention['text'])
        if rep_mention is None:
            corefs_skipped += 1
            continue
        rep_tokens = rep_mention['text'].split(' ')
        for m in mentions:
            if not replace_rep_mention and m['isRepresentativeMention']:
                continue
            m_tokens = m['text'].split(' ')
            if len(rep_tokens) < len(m_tokens):
                # tqdm.write(rep_mention['text'] + ' ------- ' + m['text'] + ' (Kth mention LONGER)')
                tqdm.write(rep_mention['text'] + ' ------- ' + m['text'])
            sent_idx = m['sentNum'] - 1
            start_idx = m['startIndex'] - 1
            end_idx = m['endIndex'] - 1
            coref_article_sent_tokens[sent_idx][start_idx] = rep_tokens
            for idx in range(start_idx+1, end_idx):
                coref_article_sent_tokens[sent_idx][idx] = []
...
def write_to_bin(url_file, out_dir, dataset_split):
  ...
  with open(out_file, 'wb') as writer:
    for idx,s in enumerate(tqdm(url_hashes)):
      ...
      coref_replaced_article, corefs_skipped, total_corefs = replace_corefs(article_sent_tokens, fixed_corefs)

      corefs_skipped_list.append(corefs_skipped)
      ...
      tf_example.features.feature['article'].bytes_list.value.extend([coref_replaced_article_text])
      tf_example.features.feature['abstract'].bytes_list.value.extend([abstract])

      num_tokens_total = sum([len(sent) for sent in coref_replaced_article])
      ...
      html_writer.write(html + '<br><br><br><br>')
