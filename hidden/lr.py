...
dataset_model = 'cnn_dm_singles_lr'
dataset_split = 'test'
model = 'ndcg5'
importance = True
...
def load_data():
    with open(os.path.join(data_dir, dataset_model, 'train.txt')) as f:
        train_data = np.load(f)
    with open(os.path.join(data_dir, dataset_model, 'val.txt')) as f:
        val_data = np.load(f)
    with open(os.path.join(data_dir, dataset_model, 'test.txt')) as f:
        test_data = np.load(f)
