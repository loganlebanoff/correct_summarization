...
out_dataset = 'cnn_dm_coref_singles'
dataset_split = 'all'
num_instances = -1,
random_seed = 123
...
def get_single_sent_features(sent_idx, sent_term_matrix, doc_vector, article_sent_tokens, mmr):
    ...
    starts_with_quote = int(does_start_with_quotation_mark(article_sent_tokens[sent_idx]))
    my_mmr = mmr[sent_idx]

    if real_values:
        return [is_sent_single, sent_idx, sent_len, starts_with_quote, my_mmr]
    else:
        ...
        return [is_sent_single] + sent_idx.tolist() + sent_len.tolist() + [starts_with_quote] + my_mmr
