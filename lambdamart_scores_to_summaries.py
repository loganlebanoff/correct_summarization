...
def get_features_all_combinations(raw_article_sents, article_sent_tokens, mmrs, single_feat_len, pair_feat_len, singles_and_pairs):
...
def generate_summary(raw_article_sents, article_sent_tokens, temp_in_path, temp_out_path, single_feat_len, pair_feat_len, singles_and_pairs):
    summary_tokens = []

    while len(summary_tokens) < 120:
        mmrs = util.calc_MMR(raw_article_sents, article_sent_tokens, summary_tokens, None)
        instances = get_features_all_combinations(raw_article_sents, article_sent_tokens, mmrs, single_feat_len, pair_feat_len, singles_and_pairs)
...
def generate_summary_importance(raw_article_sents, article_sent_tokens, temp_in_path, temp_out_path, single_feat_len, pair_feat_len, singles_and_pairs):
    tfidfs = util.get_tfidf_importances(tfidf_vectorizer, raw_article_sents)
    instances = get_features_all_combinations(raw_article_sents, article_sent_tokens, tfidfs, single_feat_len, pair_feat_len, singles_and_pairs)
...
def load_and_evaluate_example(ex):
    ...
    if importance:
        ...
        summary_sents = generate_summary(raw_article_sents, article_sent_tokens, temp_in_path, temp_out_path, single_feat_len, pair_feat_len, singles_and_pairs)
    if example_idx <= 100:
    ...
    return (groundtruth_similar_source_indices_list, similar_source_indices_list)
...
def main(unused_argv):
    ...
    print 'Converting...'
    ssi_list = list(futures.map(load_and_evaluate_example, ex_list))
    print ssi_list
    ...
    num_correct = 0
    total = 0
    for gt, sys in ssi_list:
        for ssi in sys:
            total += 1
            if ssi in gt:
                num_correct += 1
    accuracy = 1.0 * num_correct / total
    print 'Lambdamart accuracy: %.04f' % accuracy
    with open(os.path.join(my_log_dir, 'lambdamart_f1.txt'), 'w') as f:
        f.write('Lambdamart accuracy: %.04f' % accuracy)
