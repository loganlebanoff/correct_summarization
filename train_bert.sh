...
if [[ "$mode" == *"tensorboard"* ]]; then
    ...
    python bert_scores_to_summaries.py --dataset_name="$dataset_name" --singles_and_pairs="$singles_and_pairs" --use_sent_position_embeddings="$sentemb" --use_article_embedding="$artemb"  --add_hidden_layer="$plushidden"
elif [[ "$mode" == *"pg"* ]]; then
    CUDA_VISIBLE_DEVICES="$cuda" python ssi_to_pg_input.py --dataset_name="$dataset_name" --singles_and_pairs="$singles_and_pairs" --use_bert=True --use_sent_position_embeddings="$sentemb" --use_article_embedding="$artemb"  --add_hidden_layer="$plushidden"
fi
